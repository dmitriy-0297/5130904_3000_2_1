...
bool berdyanskij::Point::operator<(const Point& right)
{
...
double berdyanskij::getDistance(const Point& p1, const Point& p2)
{
...
bool berdyanskij::Polygon::operator==(const Polygon& right) const
{
...
berdyanskij::Frame berdyanskij::getFrame(const std::vector< Polygon >& data)
{
  if (data.empty())
  {
...
berdyanskij::Frame::Frame(const Point& lowerLeft, const Point& upperRight)
{
...
bool berdyanskij::Frame::containsPoint(const Point& p) const
{
  return p.x >= lowerLeft.x && p.x <= upperRight.x && p.y >= lowerLeft.y && p.y <= upperRight.y;
}

bool berdyanskij::Frame::containsPolygon(const Polygon& poly) const
{
