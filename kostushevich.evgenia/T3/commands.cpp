...
void kostushevich::area(const std::vector<Polygon> &data)
{
  auto accumalateArea_if = [](double ac, const kostushevich::Polygon &poly, std::size_t mod2, std::size_t vertices)
  {
...
void kostushevich::min(const std::vector<Polygon> &data)
{
  if (data.empty())
    throw std::runtime_error("<INVALID COMMAND>");
...
void kostushevich::max(const std::vector<Polygon> &data)
{
  if (data.empty())
    throw std::runtime_error("<INVALID COMMAND>");
...
void kostushevich::count(const std::vector<Polygon> &data)
{
  auto countPolygons_if = [](const kostushevich::Polygon &poly, std::size_t mod2, std::size_t vertices)
      {
...
void kostushevich::echo(std::vector<Polygon> &data)
{
  if (data.empty())
  {
    throw std::runtime_error("<INVALID COMMAND>");
  }
  kostushevich::Polygon poly;
  std::cin >> poly;
...
void kostushevich::maxSeq(std::vector<Polygon> &value)
{
  if (value.empty())
  {
    throw std::runtime_error("<INVALID COMMAND>");
